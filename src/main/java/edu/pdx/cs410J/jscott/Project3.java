    ...
    public static void main(String[] args) {
        ...
        for (int j = 0; j < args.length; ++j) {
            ...
            if (args[j].charAt(0) == '-') {
                ...
                } else if ((args[j]).equalsIgnoreCase("-pretty")) {
                    flags[4] = (args[j]);
        ...
        //if -textFile flag load Airline from file else create new
        Airline airline;
        if (flags[2] != null && flags[3] != null) {
            TextParser parser = new TextParser(flags[3]);
            try {
                File file = new File(flags[3]);
                if (file.isFile()) {
        ...
        //Create new Flight
        Flight flight = new Flight(commands[0], flightValue, commands[2], departureDate,
                commands[6], arrivalDate);
